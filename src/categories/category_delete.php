<?php
    ...
    // Fetch category data by ID for confirmation details
    if (isset($_GET["id"])) {
        $id = (int) $_GET["id"];

        $query = $db->prepare("SELECT nombre FROM categorias WHERE id = :id");
        $query->execute([":id" => $id]);
    ...
    if (isset($_POST["delete_submit"])) {
        ...
        $checkQuery = $db->prepare("SELECT nombre FROM categorias WHERE id = :id");
        $checkQuery->execute([":id" => $id]);

        if ($data = $checkQuery->fetch(PDO::FETCH_ASSOC)) {
            $name = $data["nombre"];

            $deleteQuery = $db->prepare("DELETE FROM categorias WHERE id = :id");
            $deleteQuery->execute([":id" => $id]);

            if ($deleteQuery->rowCount() > 0) {
                header("Location: /categories/category_list.php?deleted_category=" . urlencode($name));
                exit;
            } else {
    ...
    <div class="d-flex justify-content-center align-items-start jtech-bg">
        <div class="p-4 jtech-card" style="max-width: 600px;">
            ...
            <?php if (empty($errorMessages) && $id !== null): ?>
                ...
                <form method="post" action="/categories/category_delete.php">
                    <input type="hidden" name="id" value="<?= htmlspecialchars((string)$id) ?>">

                    <div class="d-grid gap-3">
                        <button type="submit" name="delete_submit" class="btn btn-danger fw-semibold" onclick="return confirm('¿Seguro que deseas eliminar esta categoría?');">
                            Eliminar categoría
                        </button>
